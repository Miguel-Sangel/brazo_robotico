...
{
	Level = 1;//���㲥ָ������ָ���Ӧ��
	End = 1;//Dispositivo SCS en el que primero va el Byte BAJO y despues el ALTO
}


void SCSProtocol::Host2SCS(u8 *DataL, u8* DataH, int Data) //Convierte Data en Byte alto y Byte bajo segun dispositivo
{
	if(End){
		*DataL = (Data>>8);
		*DataH = (Data&0xff);
...
int SCSProtocol::SCS2Host(u8 DataL, u8 DataH) //Devuelve Data extraido de Byte alto y Byte bajo segun dispositivo
{
	int Data;
	if(End){
		Data = DataL;
...
{
	flushSCS();
	writeBuf(ID, MemAddr, &bDat, 1, INST_WRITE);// u8 ID, u8 MemAddr, u8 *nDat, u8 nLen, u8 Fun
	return Ack(ID);
}
...
//��1�ֽڣ���ʱ����-1
int SCSProtocol::readByte(u8 ID, u8 MemAddr) // lee 1 Byte
{
	u8 bDat;
...
//��2�ֽڣ���ʱ����-1
int SCSProtocol::readWord(u8 ID, u8 MemAddr) // lee 2 Bytes Devuelve int
{	
	u8 nDat[2];
	u16 wDat;
...
{
	return readWord(ID, P_PRESENT_POSITION_L); //Devuelve posiscion 0X38
}
